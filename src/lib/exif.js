...
async function imageToWebp(media) {
  const tmpFileOut = path.join(process.cwd(), 'temp', await getRandom('webp'))
  const tmpFileIn = path.join(process.cwd(), 'temp', await getRandom('jpg'))
...
async function videoToWebp(media) {
  const tmpFileOut = path.join(process.cwd(), 'temp', await getRandom('webp'))
  const tmpFileIn = path.join(process.cwd(), 'temp', await getRandom('mp4'))
  ...
  let wMedia = /webp/.test(media.mimetype)
    ...
    : /image/.test(media.mimetype)
      ...
      : /video/.test(media.mimetype)
        ...
        : ''
  const tmpFileOut = path.join(process.cwd(), 'temp', await getRandom('webp'))
  const tmpFileIn = path.join(process.cwd(), 'temp', await getRandom('webp', '15'))
...
async function uploadFile(buffer) {
  ...
  form.append('file', buffer, await getRandom(ext))
